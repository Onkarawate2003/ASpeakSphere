...
import { Check, Ear, MessageCircle, Mic2, PenLine, Sparkles, Volume2 } from "lucide-react";
...
const focusAreaOptions: FocusAreaOption[] = [
    {
        ...
        value: "speaking_fluency",
        description: "Answer faster with fewer pauses.",
        icon: MessageCircle,
    },
    {
        label: "Pronunciation clarity",
        value: "pronunciation_clarity",
        description: "Make your words easier to understand.",
        icon: Volume2,
    },
    {
        label: "Listening confidence",
        value: "listening_confidence",
        description: "Follow conversations more comfortably.",
        icon: Ear,
    },
    {
        label: "Grammar accuracy",
        value: "grammar_accuracy",
        description: "Reduce common grammar mistakes.",
        icon: PenLine,
    },
    {
        label: "Vocabulary range",
        value: "vocabulary_range",
        description: "Use more precise and natural words.",
        icon: Sparkles,
    },
    {
        label: "Accent practice",
        value: "accent_practice",
        description: "Practice rhythm, stress, and tone.",
        icon: Mic2,
    },
];
...
export default function FocusAreasPage() {
    ...
    const isValid = data.focusAreas.length > 0;

    const toggleFocusArea = (value: string) => {
        const nextFocusAreas = data.focusAreas.includes(value)
            ? data.focusAreas.filter((focusArea) => focusArea !== value)
            : [...data.focusAreas, value];

        updateField("focusAreas", nextFocusAreas);
    };
        ...
        <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#EEF3FA] px-4 py-8 text-slate-900 sm:px-6 lg:px-8">
            ...
            >
                <div className="mx-auto flex max-w-3xl flex-col gap-8">
                    <ProgressBar currentStep={8} totalSteps={11} label="Step 8 of 11" />

                    <div className="space-y-4 text-center">
                        <h1 id="focus-areas-heading" className="text-4xl font-bold tracking-[-0.04em] text-slate-900 sm:text-5xl">
                            Which skills should we focus on?
                        </h1>
                        <p className="mx-auto max-w-[46ch] text-base leading-7 text-slate-600 sm:text-lg">
                            Pick the areas where you want the most improvement.
                        </p>
                    </div>

                    <fieldset className="grid gap-3 sm:grid-cols-2" aria-label="Choose focus areas">
                        <legend className="sr-only">Focus areas</legend>
                        {focusAreaOptions.map((option) => {
                            const selected = data.focusAreas.includes(option.value);
